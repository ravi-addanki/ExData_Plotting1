getExData <- function (){
    ...
    data1Rows <- read.csv( pipe('type data\\household_power_consumption.txt | find /C \"1/2/2007;\"'),sep=";",row.names = NULL,header=FALSE)
    ...
    data1 <- read.csv( pipe('type data\\household_power_consumption.txt | find \"1/2/2007\"'),sep=";",
    ...
    data2 <- data.frame("Date" =as.Date(data1$Date,format ="%d/%m/%Y"),"Time"= strptime( paste(data1$Date,data1$Time),format = "%d/%m/%Y %H:%M:%S"))
    #Other columns
